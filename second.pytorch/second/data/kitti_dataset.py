...
class KittiDataset(Dataset):
    ...
    def get_sensor_data(self, query):
        ...
        points = np.fromfile(
            ...
            count=-1).reshape([-1, self.NumPointFeatures])  # add
...
def create_kitti_info_file(data_path, velodyne_dir_name, num_point_features, save_path=None, relative_path=True):
    ...
    kitti_infos_train = kitti.get_kitti_image_info(
        ...
        num_point_features=num_point_features) # add
    #_calculate_num_points_in_gt(data_path, kitti_infos_train, relative_path)
    _calculate_num_points_in_gt(data_path, kitti_infos_train, relative_path, num_point_features) # add
    ...
    _calculate_num_points_in_gt(data_path, kitti_infos_val, relative_path, num_point_features)
...
def _create_reduced_point_cloud(data_path,
    ...
    for info in prog_bar(kitti_infos):
        ...
        points_v = np.fromfile(
            str(v_path), dtype=np.float32, count=-1).reshape([-1, pc_info["num_features"]])
        rect = calib['R0_rect']
        P2 = calib['P2']
        Trv2c = calib['Tr_velo_to_cam']
...
def create_reduced_point_cloud(data_path,
    ...
    #_create_reduced_point_cloud(data_path, test_info_path, save_path) # skip test data
    if with_back:
