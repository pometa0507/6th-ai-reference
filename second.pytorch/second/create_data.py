...
def kitti_data_prep(root_path, velodyne_dir_name="velodyne", num_point_features=4):
    kitti_ds.create_kitti_info_file(root_path, velodyne_dir_name, num_point_features)
    kitti_ds.create_reduced_point_cloud(root_path)
    create_groundtruth_database("KittiDataset",
        root_path, Path(root_path) / "kitti_infos_train.pkl",
